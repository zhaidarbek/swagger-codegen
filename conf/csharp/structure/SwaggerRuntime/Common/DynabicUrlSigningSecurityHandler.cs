...
namespace SwaggerRuntime.Common
{
  public class DynabicUrlSigningSecurityHandler : ISecurityHandler
  {
    ...
    {
        if (toSign.ToLower().StartsWith("http"))
        {
            return SignUrl(toSign, _clientKey, _privateKey);
        }
        else 
        {
            string signature = SignString(toSign, _privateKey);
            httpHeaders.Add(CLIENT_KEY_PARAM, _clientKey);
            httpHeaders.Add(SIGNATURE_PARAM, signature);
            return signature;
        }
    }
